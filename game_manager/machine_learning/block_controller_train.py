...
class Block_Controller(object):
    ...
    def set_parameter(self, yaml_file=None, predict_weight=None):
        ...
        # DQN の場合
            ...
            if 'self.maki_hidari_akeru_weight' in cfg["train"]:
                self.maki_hidari_akeru_weight = cfg["train"]["maki_hidari_akeru_weight"]
            else:
                self.maki_hidari_akeru_weight = 0
            print("maki_hidari_akeru_weight",self.maki_hidari_akeru_weight)

            if 'self.maki_hidari_igai_umeru_weight' in cfg["train"]:
                self.maki_hidari_igai_umeru_weight = cfg["train"]["maki_hidari_igai_umeru_weight"]
            else:
                self.maki_hidari_igai_umeru_weight = 0
            print("maki_hidari_igai_umeru_weight",self.maki_hidari_igai_umeru_weight)
            
            if 'self.maki_imino_tetris_clear_weight' in cfg["train"]:
                self.maki_imino_tetris_clear_weight = cfg["train"]["maki_imino_tetris_clear_weight"]
            else:
                self.maki_imino_tetris_clear_weight = 0
            print("maki_imino_tetris_clear_weight",self.maki_imino_tetris_clear_weight)
...
#            self.model.cuda()
        ...
        # makki debug
        log = " maki_hidari_akeru_waight: {}\n maki_hidari_igai_umeru_weight: {}\n maki_imino_tetris_clear_weight: {}\n".format(
            self.maki_hidari_akeru_weight,
            self.maki_hidari_igai_umeru_weight,
            self.maki_imino_tetris_clear_weight
        )
        # print(log)
    ...
    # ほめて伸ばす
    ####################################
    def get_maki_reward(self, reshape_board, lines_cleared):
        mask = reshape_board != 0
        c1 = self.maki_hidari_akeru_weight * self.get_maki_reward_hidari_akeru_weight(mask)
        c2 = self.maki_hidari_igai_umeru_weight * self.get_maki_hidari_igai_umeru_weight(mask)
        c3 = self.maki_imino_tetris_clear_weight * self.get_maki_imino_tetris_clear_weight(mask, lines_cleared)

        if lines_cleared >= 5:
            log = "Lopen:{:0.4f} Rfill:{:0.4f} Tetris:{:0.4f}".format(c1, c2, c3)
            print(log)
    ...
    # ・左端にブロックがないと高得点
    ####################################
    def get_maki_reward_hidari_akeru_weight(self, mask):
...
#        print(mask)
    ...
    ####################################
    # ・左端一列を除いた残りの列が埋まっているほど高得点
    ####################################
    def get_maki_hidari_igai_umeru_weight(self, mask):
#        print(mask)
        valid_blocks = 0
        ...
        for j in range(self.width):
            ...
            for i in range(self.height):
                ...
                if not col[self.height - i - 1]:
                    stop_col_count = i
#                    print("stop_col_count:", i)
                    break

            valid_blocks += stop_col_count
#            print("ttl:", valid_blocks, "+", stop_col_count)

        return valid_blocks

    ####################################
    # ・左端がてIミノできっちり埋められる場合は高得点
    #  　3列消しでもほめてあげよう！
    ####################################
    def get_maki_imino_tetris_clear_weight(self, reshape_board, lines_cleared):
        reward = 0
    ...
    # ボードを２次元化(カスタム)
    ####################################
    def get_reshape_backboard2(self, board):
        board = np.array(board)
        # 高さ, 幅で reshape
        reshape_board2 = board.reshape(self.height, self.width)
        return reshape_board2
    ...
    def step_v2(self, curr_backboard, action, curr_shape_class):
        ...
        # 形状の罰報酬
        # でこぼこ度罰
#maki        reward -= self.reward_weight[0] * bampiness
#maki        reward_bamp = -self.reward_weight[0] * bampiness
        # 最大高さ罰
#maki        if max_height > self.max_height_relax:
#maki            reward -= self.reward_weight[1] * max(0, max_height)
#maki
#maki        reward_height = -self.reward_weight[1] * max(0, max_height)
        # 穴の数罰
        ...
        # 左端以外埋めている状態報酬
#maki        reward += tetris_reward * self.tetris_fill_reward
#maki        reward_hidariigai = tetris_reward * self.tetris_fill_reward
        # 左端が高すぎる場合の罰
        ...
        reward_maki = self.get_maki_reward(reshape_board, lines_cleared)
        reward += reward_maki
        ...
        if lines_cleared >= 5:
            log = "result: {:.4f} Clines: {}\n l: {:.4f} b {:.4f} ht {:.4f} ana {:.4f} ana_b {:.4f} higai {:.4f} htaka {:.4f} maki: {:.4f}\n {}\n ======\n {}\n".format(
                self.epoch_reward,
                lines_cleared,
                reward_line,
                reward_bamp,
                ...
                reward_hidartakai,
                reward_maki,
                self.get_reshape_backboard2(curr_backboard),
                self.get_reshape_backboard2(board)
            )
            print(log)            
